...
## このサンプルで体験すること
短い実験で「ClassとObjectをVBAとPowerShellで最小実装する」を体験し、操作前後の状態やログの差を確認する。
...
### PowerShell
```powershell
class Person{[string]$Name;Person([string]$n){$this.Name=$n}};$p=[Person]::new('A');$p.Name
```

### VBA
```vb
Option Explicit
Public Sub Demo():Dim d As Object:Set d=CreateObject("Scripting.Dictionary"):d("Name")="A":Debug.Print d("Name"):End Sub
```
