...
## このサンプルで体験すること
短い実験で「For／For Each／foreachの違いを比べる」を体験し、操作前後の状態やログの差を確認する。
...
### PowerShell
```powershell
1..3|ForEach-Object{"ForEach $_"}
foreach($x in'A','B'){"foreach $x"}
```

### VBA
```vb
Option Explicit
Public Sub Demo():Dim i As Long,v As Variant:For i=1 To 3:Debug.Print i:Next:For Each v In Array("A","B"):Debug.Print v:Next:End Sub
```
