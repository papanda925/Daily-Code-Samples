...
## このサンプルで体験すること
短い実験で「localhostでHTTPとHTTPSの通信手順の違いを追う」を体験し、操作前後の状態やログの差を確認する。
...
## 最小コード
```powershell
curl.exe -I -v http://example.com 2>&1|Select -First 30
"--- HTTPS ---"
curl.exe -I -v https://example.com 2>&1|Select -First 45
```
...
## 今回触っている技術の層
```text
PowerShell → .NET / Windows networking tools → TCP/IP / Wi-Fi
```
