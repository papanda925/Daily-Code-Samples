...
## このサンプルで体験すること
短い実験で「VBAで本当の非同期HTTPを実装する」を体験し、操作前後の状態やログの差を確認する。
...
```vb
Option Explicit
' クラスモジュールで Private WithEvents http As MSXML2.XMLHTTP60 を宣言
Public Sub StartAsyncConcept():Debug.Print "Open ... True で非同期開始し、readyStateイベントで完了を受ける":End Sub
```
